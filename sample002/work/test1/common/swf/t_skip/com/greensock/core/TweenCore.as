...
{
    var vars, cachedTotalDuration, cachedDuration, _delay, cachedTimeScale, active, cachedTime, cachedTotalTime, data, cachedReversed, cachedPaused, cacheIsDirty, initted, gc, _rawPrevTime, cachedStartTime, __set__paused, __set__reversed, __get__totalDuration, timeline, _pauseTime, __get__delay, __get__duration, __set__duration, __get__currentTime, __get__totalTime, __get__startTime, __get__reversed, __get__paused, __set__currentTime, __set__delay, __set__startTime, __set__totalDuration, __set__totalTime;
    ...
    function TweenCore(duration, vars)
    {
        ...
        cachedStartTime = _loc2.cachedTotalTime + _delay;
        _loc2.addChild(this);
    ...
    function setTotalTime(time, suppressEvents)
    {
        if (timeline)
        {
            var _loc3 = _pauseTime || _pauseTime == 0 ? (_pauseTime) : (timeline.cachedTotalTime);
            if (cachedReversed)
            {
    ...
    {
        var _loc2 = Boolean(timeline != undefined && (n != cachedStartTime || gc));
        cachedStartTime = n;
        if (_loc2)
        {
            timeline.addChild(this);
    ...
    function set paused(b)
    {
        ...
        if (!b && gc)
        {
            this.setTotalTime(cachedTotalTime, false);
            this.setEnabled(true, false);
        } // end if
        //return (this.paused());
        null;
    } // End of the function
    static var version = 1.110000;
} // End of Class
